---
title: Field connection expressions
description: Learn how to use expressions to display content based on field values.
---
...
The syntax for expressions involves an Operator and a Comparison Value, both added as parameters to the conditional field connection shortcode. The `exp=` parameter is used to specify the operator, while the `value=` parameter is used to indicate the comparison value.
